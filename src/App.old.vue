<template>
  <button @click="sidebarView = !sidebarView" type="button" class="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 lg:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
  ...
  <div class="grid grid-cols-1 gap-8 lg:grid-cols-6 p-3">

    <!-- Grid 1 -- Sidebar -->
    <div class="fixed lg:relative z-30 bg-stone-50 dark:bg-stone-950 px-4 h-screen lg:h-auto w-[30vh] lg:w-auto"
    :class="[{ 'hidden': sidebarView }, { 'lg:block': !sidebarView }]">
      <!-- Sidebar Container -->
      <div>
        <!-- Overlaps Button -->
        <div class="my-2">
        ...
        <div v-for="i in 12" class="my-2">
          ...
          <label @click="overlapView = false" :for="`week${i}`" class="side-menu" :class="{ 'peer-checked:side-menu-selected': !overlapView }">
          <p>Week {{ i }}</p>
          </label>
        </div>
      </div>
      <ApexButton @click="toggleTheme">Theme</ApexButton>
    </div>
    ...
    <div v-show="!overlapView" class="main-panel">
      ...
      <Card :isFill="currentWeekChallenges.values.challenge3.legend3">
        ...
        <template #slotNBR>
          <p v-html="recolorString(`Deal 10000 damage as ${showLegends(currentWeekChallenges.values.challenge3.legend1,
...
export default {
  data() {
    return {
      ...
      textPink: 'font-bold text-pink-600',
